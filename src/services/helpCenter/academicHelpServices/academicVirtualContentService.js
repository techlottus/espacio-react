...
export const getAcademicVirtualContentHelpHttp = async () => {
  try {
    ...
    const record = recordType.find(
      (item) =>
        item.name === typesFormsHelpCenter.academicHelp.aulaContentsIssues.id
    );
...
export const postAcademicVirtualContentHttp = async (data, recordId, file) => {
  try {
    ...
    const { data: res } = await axios({
      ...
      data: {
        ...
        teacher: null,
        materialToReport: data.reportedAssignment,
