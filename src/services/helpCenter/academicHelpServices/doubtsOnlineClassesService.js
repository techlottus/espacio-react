...
export const getDoubtsOnlineClassesHttp = async () => {
  try {
    ...
    const record = recordType.find((item) => item.name === typesFormsHelpCenter.academicHelp.onlineLessons.id)
...
export const postDoubtsOnlineClassesHttp = async (data, recordId, file) => {
  try {
    ...
    const { data: res } = await axios({
      ...
      data: {
        ...
        classDate: getFormatDateHelpCenter(data.classDate),
        adeId,
        ownerId
      },
    });
