...
export const getSupportSchoolHttp = async () => {
  try {
    const { recordType } = store.getState().helpCenterProcedures;

    const record = recordType.find(
      (item) => item.value === typeIdHelpCenterProcedures.schoolService
    );
    const { value } = record;
    return {
      value,
...
export const postSupportSchoolHttp = async (data, recordId, file) => {
  try {
