...
import { typeIdHelpCenterProcedures } from "../../../types/typesHelpCenter";

const baseUrl = "";
const proceduresHelp = env.proceduresHelp;

export const getOtherAreasHelpHttp = async () => {
  try {
    const { recordType } = store.getState().helpCenterProcedures;

    const record = recordType.find(
      (item) => item.value === typeIdHelpCenterProcedures.otherAreas
    );
    const { value } = record;
    ...
    const optionsCheckboxes = record.areas.map((item) => {
...
export const postOtherAreasHttp = async (data, recordId, file) => {
  try {
    ...
    const url = `${baseUrl}${proceduresHelp}`;
    const files =
    ...
    const { data: res } = await axios({
      ...
      data: {
        ...
        areaTypeValue: data.selectCheckboxes,
